...
function Console({
  ...
  function resetPrompt() {
    ...
    let interviewerPrompt =
      "You are a job interviewer, attempting to see if the candidate is a valid fit for the job description. Your task is gather enough information about a candidate to eventually conclude if they are an attractive hire. If the job is technical based, ask technical problems to relevant to the role. You are evaluating for: clear and concise communication, problem solving/thinking, and their quality of solution. Eventually give them a mark in each category: strong hire, somewhat hire, somewhat not hire, strongly not hire.";

    if (feedback) {
      interviewerPrompt +=
        " Provide detailed criticism and deeply analyze whether candidate is good fit. Always penalize candidates for mediocre answers and grill them with harder questions, uncovering their lack of expertise.";
    } else {
      interviewerPrompt +=
    ...
    if (enabledPersonalities.length > 0) {
      ...
      interviewerPersonality =
        "Serious, stern, and concise interviewer.";
    }
    ...
    if (jobDescription.length == 0) {
      jobDescription =
        "Blank, remind the user to follow complete instructions and then ask the user for a job description";
    }

    const prompt = [
      {
        role: "system",
        content: `${interviewerPrompt} ${interviewerPersonality} ${interviewerQuestions} Limit your responses to 3 sentences. 
        Do not respond with lists or ask multiple questions at once. End every response with a question to keep the conversation going. 
        You are interviewing a candidate for the following position: ${jobDescription}.`,
      },
    ];
              ...
              return (
                <div className={styles.Message} key={index}>
                  <span className={styles.Text}>
                    Verbate: Become Interview Ready
                    <br></br>
                    <ul>
                      <li>
